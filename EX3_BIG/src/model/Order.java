...
public class Order implements Comparable<Order>, Serializable {
	...
	public Order(Customer customer, ArrayList<Dish> dishes, Delivery delivery) {
		...
		//NEW
		this.da = findDelArea(customer);
	}
	
	private DeliveryArea findDelArea(Customer customer) {
		
		for (DeliveryArea da: Main.restaurant.getAreas().values()) {
			for (Neighberhood n : da.getNeighberhoods()) {
				if (customer.getNeighberhood().equals(n)) {
					return da;
				}
			}
		}
	...
	public DeliveryArea getDa() {
		return da;
	}

	public void setDa(DeliveryArea da) {
		this.da = da;
	}
	...
	public String toString() {
		return "Order " + id + ", Customer " + customer.getFirstName();
	}
	...
	public static void fixID() {
		int maxKey = 0;
		;
		for (Integer i : Main.restaurant.getOrders().keySet()) {
			if (maxKey < i) {
				maxKey = i;
			}
		}
